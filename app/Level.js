...
const Level = ({ route, navigation }) => {
  ...
  const handleSubmitAnswer = () => {
    if (answer.toLowerCase().trim() === currentRiddle.answer.toLowerCase().trim()) {
      ...
      // Update progress to mark this level as complete
      const updatedProgress = { ...progress };
      updatedProgress[world] = [...updatedProgress[world]];
      updatedProgress[world][level - 1] = true;
  
      // Check if this is the last level of the last world
      const isWorldCompleted = updatedProgress[world].every((lvl) => lvl === true);
      const isGameCompleted = world === 4 && level === 4 && isWorldCompleted;
  
      if (isGameCompleted) {
        Alert.alert("🎉 Congratulations! 🎉", "You have successfully completed the game!", [
          {
      ...
      } else if (isWorldCompleted) {
        Alert.alert("Congratulations!", `You have completed World ${world}!`, [
          {
            text: "Go to Main Menu",
            onPress: () => {
              setAnswer('');
          ...
          world < 4
            ? {
                text: `Next World (World ${world + 1})`,
                onPress: () => {
                  setAnswer('');
                  navigation.navigate('Level', { world: world + 1, level: 1, progress: updatedProgress });
                },
              }
            : { text: "OK" }, // No next world if this is the last one
        ]);
      } else {
